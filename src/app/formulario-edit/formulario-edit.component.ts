...
import { Acto, ActoActores, Actor, ActorEditable, Datos, Escala, Escribania } from '../services/api-model';
...
export class FormularioEditComponent implements OnInit {
  ...
  ManageNameControl(index: number) {
    ...
    this.filteredOptions[index] = arrayControl.at(index).get('acto')!.valueChanges
      .pipe(
      startWith(''),
  ...
  calcularSello(index : number) : void {
    if(this.tieneSello){
      ...
      if(cant!=0){
        this.actosActores[index].actores.forEach(actor=>{
          if(actor.actor.sellos){
          actor.sello = (this.actosActores[index].datos.valor*this.actosActores[index].datos.acto.p_sellos/100)/cant;
          actor.hasSello = true;
        }})
      }
    }
    ...
    this.actosActores[index].actores.forEach(actor => {
      ...
      if(actor.actor.honorarios){
        ...
        if(this.actosActores[index].datos.acto.p_honorarios<0){
          ...
          if(this.actosActores[index].datos.valor<=this.escribaniaDatosApi[0].min_valor_escritura_he){
          ...
          actor.honorarios = this.escribaniaDatosApi[0].min_valor_honorario_escala;    
            ...
            let excedente = this.actosActores[index].datos.valor - this.escribaniaDatosApi[0].min_valor_escritura_he;
          ...
          if(this.actosActores[index].datos.valor*this.actosActores[index].datos.acto.p_honorarios/100>this.actosActores[index].datos.acto.min_honorarios)
          actor.honorarios= this.actosActores[index].datos.valor*this.actosActores[index].datos.acto.p_honorarios/100;
          else
          actor.honorarios=this.actosActores[index].datos.acto.min_honorarios;
          }
      }
  ...
  calcularDiligenciamiento(index: number) : void{
    this.actosActores[index].actores.forEach(actor => {
      if(actor.actor.diligenciamiento){
        ...
        if(this.actosActores[index].datos.valor<this.escribaniaDatosApi[0].min_valor_diligenciamiento)
          actor.diligenciamiento=this.escribaniaDatosApi[0].min_diligenciamiento;
        else{
          let excedente: number = this.actosActores[index].datos.valor-this.escribaniaDatosApi[0].min_valor_diligenciamiento;
  ...
  calcularInscripcion(index : number): void {
    this.actosActores[index].actores.forEach(actor => {
      ...
      if(actor.actor.inscripcion){
        ...
        actor.inscripcion= this.actosActores[index].datos.valor*0.002+this.escribaniaDatosApi[0].gestor;
      }
    });
  }
...
getEscalaPorcentual(valorHonorario : number) : number{
  ...
  calcularRcd(index : number): void{
      ...
      if(actor.actor.rcd){
        actor.hasRcd = true;
        if(this.actosActores[index].datos.valor*this.escribaniaDatosApi[0].p_rcd/100>this.escribaniaDatosApi[0].min_rcd)
         actor.rcd=this.actosActores[index].datos.valor*this.escribaniaDatosApi[0].p_rcd/100;
        else
  ...
  calcularGanancias(index : number) : void{
    if(this.tieneGanancia){
        this.actosActores[index].actores.forEach(actor=>{
          ...
          actor.ganancias = this.actosActores[index].datos.valor*this.actosActores[index].datos.acto.p_ganancias/100;
  ...
  calcularIti(index : number) : void{
    if(this.tieneIti){
      this.actosActores[index].actores.forEach(actor=>{
        if(actor.actor.iti){
        actor.iti = this.actosActores[index].datos.valor*this.actosActores[index].datos.acto.p_iti/100;
        actor.hasIti = true;
        }
      })
  }
  ...
  setActosActores(){
    ...
    for(let i = 0; i< arrayControl.length; i++){
      var acto : Acto = this.getActoByName(String(arrayControl.at(i).value.acto));
      // Set datos
      var dato : Datos = {
  ...
  calcular(){
    ...
    for(let i = 0; i < this.actosActores.length; i++){
      ...
      this.setCertificadosMunicipal(Number(this.formCalculador.value.municipal),i);
